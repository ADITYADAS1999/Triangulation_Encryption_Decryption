...
def encrypt():
    global total_encryption_time, final_cipher_text, xor_results, chi_square_value, degrees_of_freedom
    start_time = time.time()
    try:
        plaintext = plaintext_text.get("1.0", END).strip()
        binary_text = text_to_binary(plaintext)
        ...
        xor_results = []
        for block in blocks:
            xor_block = ''.join(str(int(block[i]) ^ 1) for i in range(len(block)))
            xor_results.append(xor_block)
            xor_result += xor_block
...
def decrypt():
    global total_decryption_time, final_cipher_text, used_options, original_source_blocks, xor_results
    ...
    try:
        ...
        # Store output to be displayed later
        output_buffer = []
            ...
            for option in ['001', '010', '011', '100']:
                ...
                if generated_target_block == xor_result:
                    if not match_found:  # Process only the first match found
                        ...
                        # Convert matched binary block to ASCII
                        ascii_value = int(generated_target_block, 2)
                        ascii_character = chr(ascii_value)
                        final_decrypted_text += ascii_character
                        final_decrypted_characters.append((generated_target_block, ascii_character))
                    
                    else:
                        output_buffer.append("Skipping further matches for this block.\n")
                else:
                    matching_status = "NOT MATCHED"
        ...
        # Perform 1's complement on each decrypted character
        final_xor_results = []
        for (binary_value, ascii_character) in final_decrypted_characters:
            complement_binary = ''.join('1' if bit == '0' else '0' for bit in binary_value)  # 1's complement
            complement_character = chr(int(complement_binary, 2))
            final_xor_results.append((binary_value, ascii_character, complement_binary, complement_character))

        # Create a string for final decrypted characters
        decrypted_characters_str = "\n".join(
            f"{binary}   : \"{char}\"   XOR_CHARACTER: {comp_binary}  :  \"{comp_char}\""
            for binary, char, comp_binary, comp_char in final_xor_results
        )

        # Store final decrypted text in buffer
        output_buffer.append("Decryption Successfully Executed\n")
        output_buffer.append("\nSummary Table:\n")
        output_buffer.append(summary_table)
        output_buffer.append(f"\n !!----------FINAL DECRYPTED CHARACTER----------!! : \n\n{decrypted_characters_str}\n\n")
        output_buffer.append(f"\n !!--------FINAL_DECRYPTED_TEXT--------!! :\n\n{final_decrypted_text}\n\n")

        # Display all buffered output at once
        triangulation_entry.insert(END, "".join(output_buffer))
        triangulation_entry.see(END)  # Ensure the latest content is visible
...
def decrypt():
    ...
    try:
        ...
        # Extract only the **XOR_CHARACTER** values for the final decrypted text
        final_decrypted_text = "".join(comp_char for _, _, _, comp_char in final_xor_results)
