...
from fractions import Fraction
from collections import Counter
import scipy.stats as stats  # Required for critical Chi-Square value

def divide_into_blocks(bit_stream, block_size=8):
    """
    Divides the bit stream into blocks of a specified size.
    """
    return [bit_stream[i:i + block_size] for i in range(0, len(bit_stream), block_size)]

def calculate_chi_square_statistic(original_counts, encrypted_counts):
    """
    Calculates the Chi-Square statistic based on observed and expected counts.
    """
    ...
    total_count = total_original + total_encrypted
    if total_count == 0:
        return 0, 0  # Return 0 chi-square if no counts are available, DOF = 0

    expected_0 = (original_counts.get('0', Fraction(0)) + encrypted_counts.get('0', Fraction(0))) / 2
    expected_1 = (original_counts.get('1', Fraction(0)) + encrypted_counts.get('1', Fraction(0))) / 2

    # Initialize the numerator
    chi_square_numerator = 0

    # Chi-square calculation
    if expected_0 > 0:
        chi_square_numerator += ((original_counts.get('0', 0) - expected_0) ** 2) / expected_0
        chi_square_numerator += ((encrypted_counts.get('0', 0) - expected_0) ** 2) / expected_0

    if expected_1 > 0:
        chi_square_numerator += ((original_counts.get('1', 0) - expected_1) ** 2) / expected_1
        chi_square_numerator += ((encrypted_counts.get('1', 0) - expected_1) ** 2) / expected_1

    return chi_square_numerator

def calculate_critical_chi_square(dof, alpha=0.05):
    """
    Calculates the Critical Chi-Square value for a given degree of freedom (DOF) and significance level (alpha).
    """
    if dof <= 0:
        return 0  # No valid degrees of freedom

    return stats.chi2.ppf(1 - alpha, dof)  # Returns the critical chi-square value

def calculate_chi_square_values(original_counts, encrypted_counts, bit_stream, alpha=0.05):
    """
    Computes both the Chi-Square statistic and the Critical Chi-Square value.
    """

    # Compute Chi-Square statistic
    chi_square_stat = calculate_chi_square_statistic(original_counts, encrypted_counts)

    # Calculate Degree of Freedom (DOF) based on total blocks
    total_blocks = max(len(divide_into_blocks(bit_stream)) - 1, 0)  # DOF = Total blocks - 1

    # Compute Critical Chi-Square value
    critical_chi_square = calculate_critical_chi_square(total_blocks, alpha)

    return float(chi_square_stat), total_blocks, critical_chi_square  # Convert to float for readability
    ...
    # **Calculate Degree of Freedom Dynamically**
    #total_blocks = max(len(divide_into_blocks(bit_stream)) - 1, 0)  # DOF = Total blocks - 1

    #return chi_square_numerator, total_blocks


    #degree_of_freedom = len(blocks) - 1
   #degree_of_freedom = len(sequences[-1]) - 1  # For each block, degree of freedom is the length of the final sequence minus one
  # Degree of freedom for binary sequence is 1
 # Degree of freedom for binary sequence is 1
...
#def calculate_degree_of_freedom(bit_stream):
    #blocks = divide_into_blocks(bit_stream)
    #total_blocks = len(blocks)
    #return max(total_blocks - 1, 0)
    #blocks = divide_into_blocks(bit_stream)
    #total_blocks = len(blocks)
...
def encrypt():
    ...
    try:
        ...
        chi_square_value, degrees_of_freedom, critical_chi_square = calculate_chi_square_values(original_counts, encrypted_counts, binary_text)
        triangulation_entry.insert(END, f"\nChi-Square Value: {float(chi_square_value):.6f}\n")
        triangulation_entry.insert(END, f"Degree of Freedom: {degrees_of_freedom}\n")
        triangulation_entry.insert(END, f"Critical Chi-Square (α=0.05): {critical_chi_square:.6f}\n")

        from tabulate import tabulate

        # Create table data
        table_data = [
            ["Chi-Square Value", f"{float(chi_square_value):.6f}"],
            ["Degree of Freedom", degrees_of_freedom],
            ["Critical Chi-Square (α=0.05)", f"{critical_chi_square:.6f}"]
        ]
        ...
        # Additional analysis on final_cipher_text
        import collections
        byte_data = final_cipher_text.encode()
        observed_freq = collections.Counter(byte_data)
        observed_freq_list = [observed_freq.get(i, 0) for i in range(256)]

        print("First 10 Byte Frequencies:", observed_freq_list[:10])
        
        total_bytes = sum(observed_freq_list)
        expected_freq = total_bytes / 256
        
        print("Expected Frequency per Byte:", expected_freq)
        
        chi_square = sum(((obs - expected_freq) ** 2) / expected_freq for obs in observed_freq_list)
        ...
        print("Critical Chi-Square Value (α = 0.05):", critical_value)
        print("\nSession Key (Decimal):\n" + str(stored_key) + "\n")
        ...
        # Create table data
        table_data_one = [
            ["Chi-Square Value", f"{float(chi_square):.6f}"],
            ["Critical Chi-Square (α=0.05)", f"{critical_value:.6f}"],
            ["Expected Frequency per Byte", f"{expected_freq:.6f}"],
            ["First 10 Byte Frequencies", f"{observed_freq_list[:10]}"]
        ]


        # Print table
        print(tabulate(table_data_one, headers=["Statistic", "Value"], tablefmt="grid"))
...
def block_frequency_test(bits, M=10):
    n = len(bits)
    N = n // M
...
def non_overlapping_template_matching_test(bits, template="111"):
    n = len(bits)
    m = len(template)
    expected = (n - m + 1) / (2 ** m)
    occurrences = sum(bits[i:i+m] == template for i in range(n - m + 1))
    chi_square = ((occurrences - expected) ** 2) / expected
    return stats.chi2.sf(chi_square, df=1)

def overlapping_template_matching_test(bits, template="111"):
      return non_overlapping_template_matching_test(bits, template)
...
def run_nist_tests(bits):
    ...
    results = {test.__name__: test(bits) for test in tests}
    return results



# Convert final cipher text to binary
cipher_binary = ''.join(format(ord(c), '08b') for c in final_cipher_text)


# Apply randomness tests
freq_test_result = frequency_test(cipher_binary)
block_freq_result = block_frequency_test(cipher_binary)
runs_test_result = runs_test(cipher_binary)
longest_run_result = longest_run_ones(cipher_binary)
rank_test_result = rank_test(cipher_binary)
discrete_fourier_transform_test_result = discrete_fourier_transform_test(cipher_binary)
cumulative_sums_result = cumulative_sums_test(cipher_binary)
approx_entropy_result = approximate_entropy_test(cipher_binary)
serial_test_result = serial_test(cipher_binary)
linear_complexity_test_result = linear_complexity_test(cipher_binary)
non_overlapping_template_matching_test_result = non_overlapping_template_matching_test(cipher_binary)
overlapping_template_matching_test_result = overlapping_template_matching_test(cipher_binary)
maurers_universal_test_result = maurers_universal_test(cipher_binary)
random_excursions_test_result = random_excursions_test(cipher_binary)
random_excursions_variant_test_result = random_excursions_variant_test(cipher_binary)


# Display results
triangulation_entry.insert(END, "\nRandomness Test Results:\n")
triangulation_entry.insert(END, f"Frequency Test: {freq_test_result:.6f}\n")
triangulation_entry.insert(END, f"Block Frequency Test: {block_freq_result:.6f}\n")
triangulation_entry.insert(END, f"Runs Test: {runs_test_result:.6f}\n")
triangulation_entry.insert(END, f"Longest Run of Ones Test: {longest_run_result:.6f}\n")
triangulation_entry.insert(END, f"Rank Test: {rank_test_result:.6f}\n")
triangulation_entry.insert(END, f"Cumulative Sums Test: {cumulative_sums_result:.6f}\n")
triangulation_entry.insert(END, f"Approximate Entropy Test: {approx_entropy_result:.6f}\n")
triangulation_entry.insert(END, f"Serial Test: {serial_test_result:.6f}\n")
triangulation_entry.insert(END, f"linear_complexity_ Test: {linear_complexity_test_result:.6f}\n")
triangulation_entry.insert(END, f"non_overlapping_template_matching_ Test: {non_overlapping_template_matching_test_result:.6f}\n")
triangulation_entry.insert(END, f"overlapping_template_matching_ Test: {overlapping_template_matching_test_result:.6f}\n")
triangulation_entry.insert(END, f"maurers_universal_test_ Test: {maurers_universal_test_result:.6f}\n")
triangulation_entry.insert(END, f"random_excursions_ Test: {random_excursions_test_result:.6f}\n")
triangulation_entry.insert(END, f" Test: {random_excursions_variant_test_result:.6f}\n")



# Print results in a tabulated format
table_data_tests = [
    ["Frequency Test", f"{freq_test_result:.6f}"],
    ["Block Frequency Test", f"{block_freq_result:.6f}"],
    ["Runs Test", f"{runs_test_result:.6f}"],
    ["Longest Run of Ones Test", f"{longest_run_result:.6f}"],
    ["Rank Test", f"{rank_test_result:.6f}"],
    ["Cumulative Sums Test", f"{cumulative_sums_result:.6f}"],
    ["Approximate Entropy Test", f"{approx_entropy_result:.6f}"],
    ["Serial Test", f"{serial_test_result:.6f}"],
]

print(tabulate(table_data_tests, headers=["Randomness Test", "Result"], tablefmt="grid"))
