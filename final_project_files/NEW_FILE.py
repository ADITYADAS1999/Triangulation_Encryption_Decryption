...
def encrypt():
    global total_encryption_time, stored_key, final_cipher_text, xor_results, chi_square_value, degrees_of_freedom, mask, original_cipher_text, xor_patterns, original_xor_pattern
    start_time = time.time()
    
    try:
        # Retrieve plaintext and convert to binary
        plaintext = plaintext_text.get("1.0", END).strip()
        binary_text = text_to_binary(plaintext)
        triangulation_entry.delete("1.0", END)
        triangulation_entry.insert(END, "Binary Input String:\n" + binary_text + "\n")
        ...
        # Generate a fully random 128-bit session key
        session_key = format(random.getrandbits(128), '0128b')  # True 128-bit key
        stored_key = int(session_key, 2)

        triangulation_entry.insert(END, "\nSession Key (Decimal):\n" + str(stored_key) + "\n")
        triangulation_entry.insert(END, "\nSession Key (Binary):\n" + session_key + "\n")

        # XOR operation with randomness enhancement
        xor_result = ''
        xor_results = []
        xor_patterns = [] 
        key_segment_index = 0
        ...
        for block in blocks:
            key_segment = session_key[key_segment_index:key_segment_index + 8]
            if len(key_segment) < 8:
                key_segment = key_segment.ljust(8, '0')
            key_segment_index = (key_segment_index + 8) % len(session_key)

            # Apply SHA-256 hashing for dynamic key transformation
            hashed_key = hashlib.sha256(key_segment.encode('utf-8')).hexdigest()
            dynamic_key = format(int(hashed_key[:2], 16), '08b').zfill(8)

            # Store original XOR pattern before adding randomness

            xor_pattern = ''.join(str(int(block[i]) ^ int(key_segment[i]))
                                  for i in range(len(block))
                                  )

            xor_patterns.append(xor_pattern)  # Save pattern for decryption

            # Perform XOR with additional random flipping
            xor_block = ''.join(
                str(int(block[i]) ^ int(dynamic_key[i]) ^ random.randint(0, 1))
                for i in range(len(block))
            )
        ...
        # Apply stronger bit scrambling
        permuted_result = ''.join(random.choice(xor_result) for _ in range(len(xor_result)))
        ...
        # Final scrambling step for additional randomness

        original_cipher_text = final_cipher_text

        mask = ''.join(str(random.randint(0, 1)) for _ in range(len(final_cipher_text)))
        final_cipher_text = ''.join(
            str(int(final_cipher_text[i]) ^ int(mask[i])) if final_cipher_text[i] in "01" else '0'
            for i in range(len(final_cipher_text))
        )

        # Store original cipher text before mask application
        triangulation_entry.insert(END, f"\nTHE ORIGINAL ENCRYPTED CIPHER TEXT BEFORE MASKING: {original_cipher_text}\n")

        # Chi-Square and frequency distribution calculations
        original_counts = count_bits(binary_text)
        encrypted_counts = count_bits(final_cipher_text)

        chi_square_value, degrees_of_freedom, critical_chi_square = calculate_chi_square_values(
            original_counts, encrypted_counts, binary_text
        )
        ...
        print("\nChi-Square Value on Byte Frequency:", chi_square)
        print("Critical Chi-Square Value (α=0.05):", critical_value)

        # Display Byte Frequency Chi-Square Table
        table_data_one = [
...
def decrypt():
    global total_decryption_time, stored_key, final_cipher_text, used_options, original_source_blocks, xor_results, mask, original_cipher_text, xor_patterns, original_xor_pattern
    ...
    try:
        ...
        # Use the stored encrypted cipher text directly
        #encrypted_text = final_cipher_text
        encrypted_text = original_cipher_text
        ...
        for block_index, block in enumerate(blocks):
            ...
            for option in ['001', '010', '011', '100']:
                ...
                # Retrieve stored XOR pattern from encryption phase
                original_xor_pattern = xor_patterns[block_index]
                ...
                if generated_target_block == xor_result:
                    if not match_found:  # Process only the first match found
                        ...
                        # Replace the matched block with the stored XOR pattern
                        #recovered_xor = xor_result  # Use the original XOR pattern instead of generated_target_block
                        
                        ## ✅ Replace the matched block with the stored XOR pattern from encryption
                        recovered_xor = original_xor_pattern
                        triangulation_entry.insert(END, f"Replacing Matched Block: {generated_target_block} → {recovered_xor}\n")
                        
                        #Retrieve the stored XOR pattern to reverse the flipping
                        #original_xor_pattern = xor_patterns[block_index]


                        #Reverse the flipping by XORing the stored pattern with the matched block
                        #recovered_xor = ''.join(
                            #str(int(generated_target_block[i]) ^ int(original_xor_pattern[i]))  # Reverse flipping
                            #for i in range(8)
                            #)
                        ...
                        session_key_segment = key_segments[block_index % len(key_segments)]
                        #resultant_xor = ''.join(str(int(generated_target_block[i]) ^ int(session_key_segment[i])) for i in range(8))
                        resultant_xor = ''.join(str(int(recovered_xor[i]) ^ int(session_key_segment[i])) for i in range(8))
                    ...
                    matching_status = "NOT MATCHED"
                    recovered_xor = generated_target_block  # If no match, keep the original

                # Add result to the summary table
                #ascii_value = int(generated_target_block, 2)
                #ascii_value = int(recovered_xor if match_found else generated_target_block, 2)
                ascii_value = int(recovered_xor, 2)
                ascii_character = chr(ascii_value)
                summary_table += f"{option:<12} | {recovered_xor:<20} | {ascii_value:<25} | {ascii_character:<20} | {matching_status}\n"
                #summary_table += f"{option:<12} | {generated_target_block:<20} | {ascii_value:<25} | {ascii_character:<20} | {matching_status}\n"
                #summary_table += f"{option:<12} | {recovered_xor if match_found else generated_target_block:<20} | {ascii_value:<25} | {ascii_character:<20} | {matching_status}\n"
