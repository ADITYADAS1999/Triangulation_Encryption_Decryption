...
def update_timer():
    global time_left, session_active, stored_key

    if time_left > 0:
        time_left -= 1
        timer_label.config(text=f"Timer: {time_left} seconds")
        root.after(1000, update_timer)  # Call the update_timer every 1 second
    else:
        # Destroy session key after timer expires
        stored_key = None
        session_active = False
        timer_label.config(text="Session Expired")
        messagebox.showinfo('Session Expired', 'The session key has expired and has been destroyed.')




def encrypt():
    global total_encryption_time, stored_key, final_cipher_text, xor_results, chi_square_value, degrees_of_freedom, time_left, session_active


    # Start the timer immediately when the encryption button is pressed
    time_left = 30  # Reset the timer to 30 seconds
    session_active = True  # Set session as active
...
# Function to start and update the 30-second timer
def update_timer():
    global time_left
    if time_left > 0:
        time_left -= 1
...
# Label to display the timer next to the session key entry
timer_label = Label(key_frame, font=('arial', 24, 'bold'), bg='red', fg='white', text=f"Timer: {time_left} seconds")
timer_label.pack(side=LEFT, padx=10)

# Start the timer when the application runs
update_timer()
